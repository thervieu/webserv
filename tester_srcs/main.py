...
def run(port: int) -> None:
	...
	print("\nCGI TESTS:")
	check_test(port, "POST /cgi/file.tester chunked 20 bits", chunked_post_no_upload)
	check_test(port, "POST /cgi/file.tester chunked 2k bits", chunked_post_no_upload_size_2k)
	check_test(port, "POST /cgi/file.tester chunked 500k bits", chunked_post_no_upload_size_500k)

	check_test(port, "POST /post-upload/another_file.tester chunked 20 bits", chunked_post_upload)
	check_test(port, "POST /post-upload/another_file.tester chunked 2k bits", chunked_post_size_2k)
	check_test(port, "POST /post-upload/another_file.tester chunked 500k bits", chunked_post_size_500k)
	check_test(port, "POST /post-upload/another_file.tester chunked 20 bits", chunked_post_upload)


	print("\nSTRESS TESTS:")
	check_test(port, "GET / 25 workers 100 times", stress_test1)
	check_test(port, "GET / 25 workers 100 times", stress_test1bis)
